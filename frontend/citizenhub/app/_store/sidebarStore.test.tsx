...
describe('sidebarStore', () => {
  beforeEach(() => {
    removeLocalStorage('UCH_SIDEBAR_OPEN');
  });
  ...
  it.each([
    ...
    'initialises isOpen to %s correctly if local storage key UCH_SIDEBAR_OPEN is %s',
    (expected, value) => {
      setLocalStorage('UCH_SIDEBAR_OPEN', value);
  ...
  it('initializes isOpen to false if local storage key UCH_SIDEBAR_OPEN is not set', () => {
    useSidebarStore.getState().init();
    const { isOpen } = useSidebarStore.getState();
    expect(isOpen).toBe(false);
  });

  it('updates local storage key UCH_SIDEBAR_OPEN when toggle is called', () => {
    useSidebarStore.getState().toggle();
    expect(getLocalStorage('UCH_SIDEBAR_OPEN')).toBe('true');
    useSidebarStore.getState().toggle();
    expect(getLocalStorage('UCH_SIDEBAR_OPEN')).toBe('false');
  });
