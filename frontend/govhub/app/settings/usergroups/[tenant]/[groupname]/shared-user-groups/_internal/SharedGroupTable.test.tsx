...
} from '@/app/settings/usergroups/[tenant]/[groupname]/shared-user-groups/_internal/SharedGroupTable';
import { ShareGroupTableTestIds as TableTestIds } from '@/app/settings/usergroups/[tenant]/[groupname]/shared-user-groups/testIds';
...
const deleteUserGroupPermissionMock =
  deleteUserGroupPermission as unknown as FuncMock<
    typeof deleteUserGroupPermission
  >;
const updateUserGroupPermissionMock =
  updateUserGroupPermission as unknown as FuncMock<
    typeof updateUserGroupPermission
  >;
jest.mock(
...
const renderComp = createRender(SharedGroupTable, {
  tenant: TENANT,
...
describe('content', () => {
  it('table contains as many entries as shared groups given', async () => {
    renderComp({ sharedGroups: GROUPS });
...
describe('permission', () => {
  const renderPermissionBadge = createRender(InternalPermissionBadge, {
    tenant: TENANT,
    groupName: GROUP_NAME,
    sharedGroup: GROUPS[GROUP_INDEX],
  });
  ...
  describe('structure', () => {
    ...
    it.each(SCOPE_TEST_CASES)(
      ...
      (expectedLabel, permission) => {
        ...
        renderComp({ sharedGroups: [testGroup] });
  ...
  describe('update', () => {
    ...
    it('permission badge allows updating user group permission for specific user group', async () => {
      const successState = { data: {} };
      updateUserGroupPermissionMock.mockResolvedValueOnce(successState);
      renderPermissionBadge({
        tenant: TENANT,
        groupName: GROUP_NAME,
        sharedGroup: GROUPS[GROUP_INDEX],
      ...
      expect(updateUserGroupPermissionMock).toHaveBeenCalledWith(
        TENANT,
        GROUP_NAME,
        GROUPS[GROUP_INDEX],
        expect.anything(),
    ...
    it.each(ERROR_MAPPING_TEST_CASES)(
      ...
      async ({ errors }) => {
        updateUserGroupPermissionMock.mockResolvedValueOnce({ errors });
        renderPermissionBadge();
...
describe('delete badge', () => {
  ...
  describe('structure', () => {
    it('delete badge has correct tooltip', () => {
      ...
      expect(tooltip).toHaveTextContent('Berechtigung entfernen');
    });
  ...
  describe('delete', () => {
    ...
    it('delete badge allows deleting user group permission for specific user group', async () => {
      const successState = { data: {} };
      deleteUserGroupPermissionMock.mockResolvedValueOnce(successState);
      renderDeleteBadge({
        tenant: TENANT,
        groupName: GROUP_NAME,
        sharedGroup: GROUPS[GROUP_INDEX],
    ...
    it('modal shows error messages if deletion fails', async () => {
      ...
      deleteUserGroupPermissionMock.mockResolvedValueOnce({
    ...
    it('shows error toast if deletion rejected', async () => {
      deleteUserGroupPermissionMock.mockRejectedValueOnce(
